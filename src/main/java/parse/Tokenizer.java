...
public class Tokenizer implements Iterator<Token> {
	...
	private int nextChar(boolean exceptionOnEOF) throws IOException, EOFException {
		...
		while (c == '/') // when we encounter
		{
			br.mark(5); // mark the IO position
			if (br.read() == '/') {
				br.reset(); // this will let the reader skip the line which has a prefix of "//"
				br.readLine();
				c = br.read();
			} else {
				br.reset();
				break;// do not forget to reset to the origin place.
			}
		}
